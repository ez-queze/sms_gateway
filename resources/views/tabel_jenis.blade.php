...
<div class="w-full bg-white rounded-md">
    <div class="flex justify-end w-full px-2 mt-2">
        ...
        <form action="{{ route('dashboard.cari_jenis') }}" method="POST">
            @csrf
    ...
    <tbody class="text-sm font-normal text-gray-700">
        ...
        @foreach ($jenis_pelanggarans as $item)
           <tr class="py-10 border-b border-gray-200 hover:bg-gray-100">
                <td class="text-center w-2/12 py-4">{{ $loop->iteration }}</td>
                <td class="text-center w-2/12 py-4">{{ $item->nama_pelanggaran}}</td>
                <td class="text-center w-2/12 py-4">{{ $item->kategori }}</td>
                <td class="text-center w-2/12 py-4">{{ $item->poin }}</td>
                <td class="text-center w-2/12 py-4">
                   <div class="space-y-10 md:space-x-2 md:space-y-0">
                   <a href="/dashboard/ubah/jenis_pelanggaran/{{ $item->id }}" target="_parent">
                        <button type="button" class="w-24 p-1 m-2 font-semibold text-white transition-all duration-300 bg-yellow-400 rounded-full shadow-lg hover:bg-yellow-600 focus:outline-none focus:ring hover:shadow-none">Ubah</button>
                       </a>
                   </div>
               </td>
           </tr>
